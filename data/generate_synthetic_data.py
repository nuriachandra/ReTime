...
def generate_sine_wave(
    length: int, amplitude: float = 1.0, frequency: float = 0.1, phase: float = 0.0, noise_level: float = 0.0
) -> np.ndarray:
    """
...
def generate_linear_trend(
    length: int, slope: float = 0.01, intercept: float = 0.0, noise_level: float = 0.0
) -> np.ndarray:
    """
...
def generate_seasonal(
    length: int, period: int = 24, amplitude: float = 1.0, pattern: str = "sine", noise_level: float = 0.0
) -> np.ndarray:
    """
...
def generate_random_walk(
    length: int, drift: float = 0.0, volatility: float = 1.0, start_value: float = 0.0
) -> np.ndarray:
    """
...
def generate_ar_process(
    length: int, ar_params: List[float], mean: float = 0.0, noise_level: float = 1.0, burn_in: int = 100
) -> np.ndarray:
    """
...
def create_batch_sequences(
    series: np.ndarray, n_seqs: int, seq_length: int, horizon_length: int, stride: int = 1
) -> np.ndarray:
    """
...
def main():
    ...
    parser.add_argument(
        ...
        type=str,
        default="constant",
        choices=["constant", "sine", "linear", "seasonal", "random_walk", "ar", "complex"],
    ...
    print(
        f"Based on n_seqs={args.n_seqs}, seq_length={args.seq_length}, "
    ...
    sequences = create_batch_sequences(series, args.n_seqs, args.seq_length, args.horizon_length, args.stride)
    ...
    print(
        f"Output shape: {sequences.shape} ({args.n_seqs} sequences, each {args.seq_length + args.horizon_length} long)"
    )
